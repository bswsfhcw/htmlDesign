...
function setTopoData(result) {
	scene.clear(); //加载前清空画板
	var zoom = result.zoom == undefined ? 1 : result.zoom ;
...
function zTreeOnDrop(event, treeId, treeNodes) {
	...
	if(treeNode.type == 2 || treeNode.type == 3) { //开关
		...
		var imgHeight = imgH;
		//横竖
		if($("input[name='vhRadio']:checked").val() == 'hr' && img.indexOf("-v.") == -1) {
			img = img.replace(".", "-v.");
			imgWidth = imgH;
			imgHeight = imgW;
		} //
		var node = newNode(_add_x, _add_y, imgWidth, imgHeight, treeNode.name, "switch", img, 0, treeNode.id);
		...
		var node = newNode(_add_x, _add_y, imgWidth, imgHeight, treeNode.name, "transformer", img, 0, treeNode.id);
		_newNodeImgPath = null;
		_entityType = null;
...
function onClick(event, treeId, treeNode) {
	if(treeNode.type == 1) { //变电站
		console.log("111");
		$('#findText').val(treeNode.name);
		$('#findButton').click();
	}
}
...
function saveTopo() {
	...
	translateX = 0;
	translateY = 0;
	console.log("scene.scaleX:"+scene.scaleX);
	var emslist = [];
	scene.findElements(function(e) {
		var node = e;
